...
const uri = process.env.DATABASE_URL || "mongodb://localhost:27017/rim-ebay";

async function ensureCollection(db: any, name: string, validator?: any) {
  const exists = await db.listCollections({ name }).hasNext();
  if (!exists) {
    await db.createCollection(name, validator ? { validator, validationLevel: "moderate" } : undefined);
...
async function ensureIndexes(db: any) {
  ...
  // Annonce images (table de jonction)
...
async function run() {
  ...
  const db = client.db(); // rim-ebay choisi via l’URI
  ...
  const userValidator = {
    $jsonSchema: {
      ...
      properties: {
        ...
        verifyToken: { bsonType: ["string", "null"] },            // ← optionnel
  ...
  const passwordResetValidator = {
    $jsonSchema: {
      ...
      properties: {
        userId: { bsonType: "string" }, // String selon Prisma
  ...
  const optionsValidator = {
    $jsonSchema: {
      ...
      properties: {
        ...
        tag: { bsonType: "string" },
        createdAt: { bsonType: "date" },
        parentID: { bsonType: ["string", "null"] }, // String selon Prisma
      },
      additionalProperties: true,
    },
  };
  ...
  // haveImage devient requis (protection contre les documents incomplets)
  const annonceValidator = {
    $jsonSchema: {
      ...
      properties: {
        ...
        userId: { bsonType: "string" },
        subcategorieId: { bsonType: "string" }, // orthographe comme le modèle
  ...
  // Pivot: IDs en ObjectId (conforme @db.ObjectId)
